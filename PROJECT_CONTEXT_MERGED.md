...
Current Version Label: `v13.4` (`The first light / ÂàùË¶ã‰πãÂÖâ`)
...
### New in v13 (Render Stage 2 - Real Sprite Pipeline, 24px) ‚Äî Codename: The first light / ÂàùË¶ã‰πãÂÖâ
...
- `ver13/` created as a v13-series snapshot (codename: `The first light / ÂàùË¶ã‰πãÂÖâ`; includes `src/`, `assets/`, `index.html`, `index_single.html`).
...
- Added friendly combat unit key `saber` (UI label: `æ‘§h`) with configurable initial count before map generation.
- Added enemy camp marker at top-left area as spawn origin only (no HP / non-attackable gameplay object).
- Added `•Õ¶®ºƒ§H` button to spawn one enemy `raider` (`®BßL`) near enemy camp per click.
...
Release label:
- `v14` (`Iron Frontier / ≈KæW´eΩu`)

Release scope:
- Added friendly melee unit `saber` (`æ‘§h`) with configurable initial count.
- Added top-left enemy camp as raider spawn origin.
- Added `•Õ¶®ºƒ§H` button to spawn enemy `raider` (`®BßL`) units.
...
Codename:
- `Iron Frontier / ≈KæW´eΩu`

## 13) 2026-03-02 v14 Codename Rename Note

- v14 codename was renamed from the previous working label to `∏π®§≈T∞_`.
...
- v14 public codename should be written as `Call to Arms / ∏π®§≈T∞_`.
