...
- Added friendly combat unit key `saber` (UI label: `�Ԥh`) with configurable initial count before map generation.
- Added enemy camp marker at top-left area as spawn origin only (no HP / non-attackable gameplay object).
- Added `�ͦ��ĤH` button to spawn one enemy `raider` (`�B�L`) near enemy camp per click.
